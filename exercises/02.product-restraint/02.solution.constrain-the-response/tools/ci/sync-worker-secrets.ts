...
async function runWranglerSecretBulk(options: CliOptions, dotenvText: string) {
	const wranglerArgs = ['secret', 'bulk']
	...
	wranglerArgs.push(secretsFilePath)
	if (options.env !== undefined) {
		wranglerArgs.push('--env', options.env)
	}
	if (options.name) {
		wranglerArgs.push('--name', options.name)
	}
	if (options.config) {
		wranglerArgs.push('--config', options.config)
	}

	try {
		const proc = spawn(resolveWranglerCommand(), resolveWranglerArgs(wranglerArgs), {
...
function resolveWranglerArgs(args: Array<string>) {
	const localWranglerPath = join(
		process.cwd(),
		'node_modules',
		'.bin',
	...
	return existsSync(localWranglerPath) ? args : ['-y', 'wrangler', ...args]
}
