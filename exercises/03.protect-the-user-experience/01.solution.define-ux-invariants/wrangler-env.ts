...
const wranglerLauncher = existsSync(localWranglerPath)
	? { command: localWranglerPath, args: commandArgs }
	: {
			command: process.platform === 'win32' ? 'npx.cmd' : 'npx',
			args: ['-y', 'wrangler', ...commandArgs],
		}

const proc = spawn(wranglerLauncher.command, wranglerLauncher.args, {
	stdio: 'inherit',
	env: processEnv,
})
