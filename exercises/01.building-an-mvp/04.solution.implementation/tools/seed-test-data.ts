...
function runWrangler(args: Array<string>) {
	...
	const wranglerCommand = existsSync(localWranglerPath)
		? localWranglerPath
			...
			default: {
				if (arg.startsWith('-')) {
					fail(
						[
							`Unknown flag: ${arg}`,
							'Usage: node tools/seed-test-data.ts [--local|--remote] [--config <path>] [--persist-to <path>] [--email <email>] [--username <username>] [--password <password>]',
						].join('\n'),
					)
				}
			}
